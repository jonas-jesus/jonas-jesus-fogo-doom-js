...
function updateFireIntensityPerPixel(currentPixelIndex){
    const belowPixelIntensit = currentPixelIndex + fireHeight
    if(belowPixelIntensit >= fireHeight * fireWidth){
        return
    }

    const decay = Math.floor(Math.random()*3)
    const belowPixelFireIntensity = firePixelsArray[belowPixelIntensit]
